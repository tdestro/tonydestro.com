package org.papervision3d.objects.parsers
{
	...
	import com.healthysight.events.SceneEvent;
	...
	public class DAE extends DisplayObject3D implements IAnimationDataProvider, IAnimatable
	{
		...
		public var p:*; ///// the parent displayobject			
		...
		protected var totalLoadedMaterials:int;
		
		protected var currentLoadedMaterials:int;
		 ...
		 */ 
		public function DAE(autoPlay:Boolean=true, name:String=null, loop:Boolean=false, _p:*=null)
		{
			super(name);
			p=_p;
			_autoPlay = autoPlay;
		 ...
		 */ 
		 ////////////////////////////////////////////////////////////////////////////////////////////////////
		 // changing image load functionality from a queue to all loading at once, so we can use a preloader
		 ////////////////////////////////////////////////////////////////////////////////////////////////////

		 
		protected function loadAllMaterials():void{
			currentLoadedMaterials = 0;
			totalLoadedMaterials = _queuedMaterials.length;
			
			for (var i:int=0; i< _queuedMaterials.length; i++){
				var data:* = _queuedMaterials[i];
				...
				material.addEventListener(FileLoadEvent.LOAD_PROGRESS, materialFileProgress);					
				material.addEventListener(FileLoadEvent.LOAD_COMPLETE, materialFileLoaded);
		...
		protected function materialFileLoaded(event:FileLoadEvent):void{
			p.dispatchEvent(new SceneEvent({evtType:SceneEvent.LOADED, name:""}));

			event.currentTarget.removeEventListener(FileLoadEvent.LOAD_COMPLETE, materialFileLoaded);
			//event.currentTarget.removeEventListener(FileLoadEvent.LOAD_PROGRESS, materialFileProgress);	
			event.currentTarget.removeEventListener(FileLoadEvent.LOAD_ERROR, onMaterialError);					
			
			currentLoadedMaterials++;
			if(currentLoadedMaterials == totalLoadedMaterials){
				dispatchEvent(new FileLoadEvent(FileLoadEvent.COLLADA_MATERIALS_DONE, this.filename));
				onMaterialsLoaded();				
			}
		}
		
		protected function materialFileProgress(e:FileLoadEvent):void{
						
			//TODO: dispatchevent to root
			if(p != null){
				trace("DISPATCHING SCENE EVENT");
				var cm:int=currentLoadedMaterials+1;
				
				p.dispatchEvent(new SceneEvent({evtType:SceneEvent.PROGRESS, name:"", bytesLoaded:e.bytesLoaded, bytesTotal:e.bytesTotal, currentAsset:cm, totalAssets:totalLoadedMaterials}));
			}
			var currImageNum = currentLoadedMaterials + 1;
			
			trace("image " + currImageNum + " of " + totalLoadedMaterials);

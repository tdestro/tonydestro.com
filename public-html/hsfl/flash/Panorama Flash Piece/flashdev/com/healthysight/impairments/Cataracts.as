package com.healthysight.impairments{
	...
	public class Cataracts extends Sprite{
		...
		public function Cataracts(_container:PanoramaModule){
			...
			var url:String = "images/disease_pngs/cataracts.png";
			var urlReq:URLRequest = new URLRequest(url);
			ldr.load(urlReq);
			
			container.loaderBg_mc.dispatchEvent(new ImpairmentEvent({evtType:ImpairmentEvent.LOAD_START, name:"cataracts"}));
		...
		public function init(){
			...
			blurMask_mc.blendMode = BlendMode.ALPHA;
			blurMask_mc.cacheAsBitmap = true;			
			filter_bmp.cacheAsBitmap = true;
		...
		private function initHandler(e:Event){
			container.loaderBg_mc.dispatchEvent(new ImpairmentEvent({evtType:ImpairmentEvent.LOADED, name:"cataracts"}));
			img = e.currentTarget.content;
		}
		
		private function progressHandler(e:ProgressEvent){
			container.loaderBg_mc.dispatchEvent(new ImpairmentEvent({evtType:ImpairmentEvent.PROGRESS, name:"cataracts", bytesLoaded:e.bytesLoaded, bytesTotal:e.bytesTotal}));
		}
		
		
		
		/////////  test copying bitmapdata of screen  /////////////
